...
for (const preview of ["", "?preview=studio-full"])
  test(`account progress survives a second device and failed saves never report success ${preview}`, async ({
    page,
    browser,
    baseURL,
  }) => {
    ...
    try {
      ...
      await other.goto(`${baseURL}/${preview}#account`);
      ...
      expect((await browserApi(other, "/api/auth/me")).user).toBeNull();
